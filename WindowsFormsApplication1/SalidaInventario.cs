...
namespace WindowsFormsApplication1
{
    public partial class SalidaInventario : Form
    {
        ...
        private void showIndex_Click2(object sender, EventArgs e)
        {
            ...
            if (unidades.ShowDialog() == DialogResult.OK)
            {
                if (!unidades.textBox2.Text.Equals(""))
                    ...
                    mov.unidades = 1;
                if (verificarExistencia(pr.id,mov.unidades))
                {
                    prods.Add(pr);
                    movimientos.Add(mov);
                }
        ...
        private bool verificarExistencia(int id, int unidades)
        {
            for(int i = 0; i < tExistencias.existencias.Count; i++)
            {
                if(tExistencias.existencias.ElementAt(i).idProducto == id)
                {
                    if (tExistencias.existencias.ElementAt(i).unidades >= unidades)
                        return true; 
                    MessageBox.Show("Usted solo tiene " + tExistencias.existencias.ElementAt(i).unidades + " Unidades en Inventario");
                    return false;
                }
            }
        ...
        private bool enviarCompra()
        {
            ...
            if (idDocumento != -1)
            {
                ...
                for (int i = 0; i < movimientos.Count; i++)
                {
                    ...
                    mov.unidades = movimientos.ElementAt(i).unidades * -1;
                    mov.idConcepto = 32;
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            if (prods.Count > 0)
            {
                ...
                {
                    reiniciarGridView();
                    MessageBox.Show("Salida Inventario Realizada", "Mensaje");
                }
            }
            else
            {
